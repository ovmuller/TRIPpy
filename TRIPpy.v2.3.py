...
def main():
    ...
    if calibrate:
        ...
        if c_start_tier == -1 or c_start_tier > c_max_tier:
            ...
            for t in range(c_max_tier, start_tier, -1):
                ...
                with open(filename, "r") as f:
                    for line in f:
                        match = re.search(r"Best parameterisation for stid (\d+): speed=([\d\.]+), meander=([\d\.]+)", line)
                        if match:
        ...
        for t in range(start_tier, -1, -1):    
            ...
            tier_calib_simulations = run_calibration(tier_observations, 
                                                     stid_s_m, 
...
def set_configuration():
    ...
    global c_obs_path, c_obs_suffix, c_dir_seq_ifile, c_dir_seq_ofile, \
           ...
           c_speed, c_meander, c_start_tier, c_speed_meander_ofile, c_output_path, \
...
def plot_comparison(stid, observed, simulations):
    '''
    ...
    dates = np.arange(np.datetime64(f'{c_start}-01'), 
                      np.datetime64(f'{c_end+1}-01'), dtype='datetime64[M]')
    ...
    #ax1.legend(loc='upper left', bbox_to_anchor=(1, 1))2.
...
def run_calibration(observations,stid_s_m,basin_masks,cum_tiers):
    ...
    input:
        ...
        stid_s_m: dict - A dictionary containing station IDs as keys
                  ...
                  parameters (speed and meander) for that station’s 
                  contributing grid-cells.
        basin_masks: 2darray - Domain to run the current calibration tier.
        cum_tiers: dict, optional - A dictionary containing calibration tiers as 
                   keys and the stid of each tier. It includes the tier to be 
                   calibrated in this iteration, along with the tiers already 
    ...
    for s in c_speed:
        for m in c_meander:
            ...
            for stid in observations.keys():
                ...
                simulation[stid] = {'latitude': observations[stid]['latitude'],
            ...
            speed_param, meander_param = set_speed_meander_params(nlat, nlon, 
                                                                  0.5, 1.4, 
                                                                  True,
                                                                  stid_s_m, 
                                                                  basin_masks, 
            ...
            # RUN THE SIMULATION
            simulation = run_simulation(c_params_path, c_forcing_path, c_forcing_prefix, \
    ...
    return calib_simulations

def select_best_params(calib_simulations, observations, tier):
    '''
    ...
    with open(c_output_path + f'calibration_tier_{tier}.txt', 'w') as f:
        ...
        for stid, data in calib_simulations.items():
            ...
            for simul in data:
                ...
                if args.verbose:
                    print(f"{simul['speed']:<10}{simul['meander']:<10}"
                          f"{simul['correlation']:.3f}")
                f.write(f"{simul['speed']:<10}{simul['meander']:<10}"
                        f"{simul['correlation']:.3f}\n")
            ...
            plot_comparison(stid, observation['observed_data'], data)
